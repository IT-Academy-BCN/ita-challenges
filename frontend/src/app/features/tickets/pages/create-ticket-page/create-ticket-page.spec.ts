...
describe('CreateTicketPage', () => {
  ...
  beforeEach(async () => {
    mockTicketService = {
      create: () => of({ id:'1', userId: 'one', title: '', description: '', status: TicketStatus.OPEN, comment: null, assignedTo: null, createdAt: new Date(), updatedAt: new Date()  })
    };
  ...
  it('should call ticketApiService.create when call onSubmit with correct data', () => {
    ...
    vi.spyOn(mockTicketService, 'create').mockReturnValue(of({ id: '1', userId: 'usuari', ...testData, status: TicketStatus.OPEN, comment: null, assignedTo: null, createdAt: new Date(), updatedAt: new Date() }));
    component.onSubmit();
